...
The Homebrew handoff is independent of signing ownership. It snapshots the tap workflow's runs, dispatches `update-formula.yml` in the resolved `homebrew-tap` with the live contract—`formula`, canonical `tag`, and source `repository`—then requires exactly one new matching run from the token actor and waits for it to succeed. Only then does it poll the tap's `Formula/<formula>.rb`, within one 15-minute deadline. A parser-backed closed fetchable grammar rejects alternate setters, reflection, dynamic dispatch, heredoc/comment decoys, and non-literal URL/hash forms without executing tap code. Every accepted formula URL must name an exact release asset from that repository and tag, and every adjacent `sha256` must equal the digest in the two byte-identical verifier attestations. A successful dispatcher run alone is never accepted as proof.

The live three-field contract cannot carry the numeric release ID, source tag object, verifier attestation, or a correlation ID. Residual gap: run snapshot, title, and token actor cannot cryptographically distinguish an identical same-actor dispatch that appears after the final ambiguity check, nor bind the resulting tap commit directly to the numeric GitHub release. The handoff fails on multiple observed matches and proves the post-run formula state is content-bound to the independently verified release asset names and hashes. The final stage opens a PR adding the next `## Unreleased` section.
...
This runs actionlint on reusable, CI, and example workflows; parses every YAML document with Psych safe loading; executes adversarial frozen-tag, draft-binding, Homebrew input-matrix, and post-dispatch formula-binding scenarios; and enforces the required job/input topology, actions-read-only dual-architecture verifier, publisher hash binding, and immutable action references.
